...
public class SeedData
{
    public static async Task EnsureSeedData(WebApplication app)
    {
        using (var scope = app.Services.GetRequiredService<IServiceScopeFactory>().CreateScope())
        {
            ...
            {
                var invoice1 = new Invoice(DateTime.Now.Subtract(TimeSpan.FromDays(-10)), InvoiceStatus.Draft);
                invoice1.AddItem(ProductType.Good, "Item 1", 30, "pc", 0.25, 2);

                context.Invoices.Add(invoice1);

                var invoice2 = new Invoice(DateTime.Now, InvoiceStatus.Draft);
                ...
                var invoice3 = new Invoice(DateTime.Now, InvoiceStatus.Draft);
